...
var graph = new joint.dia.Graph(); //   With these two lines of code, we're setting up the environment to work with JointJS and an Entity-Relationship Diagram (ERD). 
                                 //  We can then use the erd object to create specific ERD shapes, and the graph object to manage the structure and relationships between these shapes in your diagram.
...
var paper = new joint.dia.Paper({
    ...
    height: 600,
    model: graph,
    linkPinning: false,  //  This option specifies whether or not links (connections) can be pinned in place. Setting it to false means that links cannot be pinned in place.
    highlighting: false,  // This option controls whether highlighting of elements (e.g., when you hover over them) is enabled. Setting it to false disables element highlighting.
...
}); // The element.getConnectionPoint(line.start) is called to determine the connection point based on the starting point of the line. If that's not available, it falls back to using the center of the element's bounding box.

// Custom highlighter - display an outline around each element that fits its shape.

var highlighter = V('path', { // The V() function in JointJS is used to create a vector element.
    'stroke': '#e9fc03',
    'stroke-width': '2px',
    'fill': 'transparent',
    'pointer-events': 'none'
